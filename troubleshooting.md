...
Режим **--mixed** используется по умолчанию, т.е. его не обязательно явно указывать при выполнении команды `git reset`, можно просто написать
...
* эта команда не отменяет действия коммитов, которые находятся между HEAD и коммитом, который вы укажете в `git revert`, поэтому могут возникать конфликты
...
После ввода команды откроется текстовый редактор для ввода сообщения, которое будет применено к новому коммиту, который будет создан командой `git revert`. Это аналогично флагу `-m`, которую мы используем в `git commit`.
